...
const ProfileMenu = ({ user, onClose }: { user: User; onClose: () => void }) => {
    return (
        <div className="bg-indigo-950/90 rounded-t-2xl rounded-b-none sm:rounded-2xl mb-1 text-white relative max-h-96 overflow-hidden" style={{ width: 'var(--menu-width)' }}>
...
export const AppMenu = ({ user }: AppSidebarProps) => {
    const [activeMenuItem, setActiveMenuItem] = useState<string | undefined | null>();
        ...
        <div className="absolute z-50 bottom-0 left-0 w-full sm:left-4 sm:w-auto sm:translate-x-0 sm:rounded-2xl sm:m-4 m-0 rounded-none">
            ...
            >
                ...
                <div className="sm:hidden">
                    ...
                    {showMobileProfile && (
                        <div className="absolute bottom-full left-0 mb-1 w-full" style={{ width: `${menuWidth}px` }}>
                            <ProfileMenu user={user} onClose={() => setShowMobileProfile(false)} />
                        </div>
                    )}
                </div>
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent side="top" className="min-w-fit flex flex-col sm:flex-row">
                        {tools.map((tool) => (
                            <Tooltip key={tool.id}>
                                <TooltipTrigger asChild>
                                    <DropdownMenuItem
